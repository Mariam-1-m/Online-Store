...
function OrderNotesCard({setOrderNotes}){
    return(
...
<div className="w-[98%] h-20 flex items-center p-4 border border-indigo-600 dark:border-indigo-500 dark:bg-slate-800/90 bg-indigo-50 rounded-xl">
  ...
  <div className="w-full">
    <textarea onChange={(e)=>setOrderNotes(e.target.value)} className="w-full h-full outline-0 p-0 text-sm" placeholder="Any special instructions for your order..." />
  </div>
</div>
    </div>
    );
